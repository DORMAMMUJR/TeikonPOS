...
app.post('/api/stores/new', authenticateToken, async (req, res) => {
    try {
        ...
        res.status(201).json({
            ...
            organizationId: store.organizationId
// ==========================================
// ENDPOINTS DE TURNO DE CAJA (CASH SHIFT)
// ==========================================
...
app.post('/api/shifts/start', authenticateToken, async (req, res) => {
                try {
                    ...
                    // Verificar si ya hay un turno abierto para este usuario/tienda
            ...
            // GET /api/shifts/current - Obtener Turno Activo
            ...
            // POST /api/shifts/end - Cerrar Turno
            app.post('/api/shifts/end', authenticateToken, async (req, res) => {
                try {
                    const { shiftId, finalBalance, cashSales, cardSales, transferSales, expenses, notes } = req.body;
                    ...
                    // Calcular esperados y diferencia
                    // Monto Esperado = Monto Inicial + Ventas Efectivo - Gastos (si se pagaron en efectivo)
                    // Nota: Asumimos que los gastos salen de la caja de efectivo por defecto para este cálculo simple,
                    // o que el frontend ya pasó el neto.

                    // Recalcularemos basado en lo que manda el frontend o backend?
                    // El frontend suele tener la verdad de qué ventas ocurrieron en local si hubo offline, 
                    // pero idealmente el backend debería sumar las ventas de la DB.
                    // Por simplicidad y robustez inicial, confiamos en los totales calculados que envía el frontend
                    // que (en teoría) coinciden con el backend si todo está sincronizado.

                    const expectedBalance = parseFloat(shift.montoInicial) + parseFloat(cashSales) - parseFloat(expenses);
                    const difference = parseFloat(finalBalance) - expectedBalance;

                    await shift.update({
                        cierre: new Date(),
                        montoReal: finalBalance,
                        ventasEfectivo: cashSales,
                        ventasTarjeta: cardSales,
                        ventasTransferencia: transferSales,
                        gastos: expenses,
                        montoEsperado: expectedBalance,
