...
app.get('/api/dashboard/summary', authenticateToken, async (req, res) => {
    try {
        ...
        if (req.role !== 'SUPER_ADMIN') {
            whereProduct.storeId = req.storeId;
        }

        // Calcular inversión: SUM(costPrice * stock)
        // Nota: Sequelize no soporta operaciones aritméticas directas en SUM fácilmente sin literal, 
        // así que iteramos o usamos literal. Usaremos iteración para seguridad DB agnóstica por ahora o literal si es PG.
        // Dado que usamos PG, podemos usar literal.
        ...
        let dailyTarget = 0;
        if (req.role !== 'SUPER_ADMIN') {
            const config = await StoreConfig.findOne({ where: { storeId: req.storeId } });
