...
const POS: React.FC = () => {
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-6 h-[calc(100vh-280px)]">
      ...
      <div className="lg:w-2/3 flex flex-col card-premium overflow-hidden border-t-4 border-t-brand-emerald bg-white dark:bg-slate-900 shadow-xl">
        <div className="p-5 border-b border-brand-border bg-slate-50 dark:bg-emerald-950/20">
          <div className="relative group">
            ...
            <input 
              ...
              placeholder="SCANNER LISTO... INGRESE SKU O NOMBRE"
              value={searchTerm}
      ...
      <div className="lg:w-1/3 flex flex-col card-premium overflow-hidden border-t-4 border-t-brand-emerald shadow-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4 no-scrollbar">
          {cart.map(item => {
            ...
            // VENTA FLEXIBLE: Alerta visual para stock negativo sin bloquear la operación
            ...
            return (
              <div 
                ...
                className={`flex flex-col p-5 rounded-[1.8rem] border-2 transition-all duration-300 shadow-sm ${
                  isOverselling 
                  ? 'border-red-500 bg-red-500/5 dark:bg-red-500/10 shadow-[0_0_20px_rgba(239,68,68,0.1)]' 
              ...
              >
                <div className="flex justify-between items-start gap-4 mb-4">
                    ...
                    <div className="flex flex-wrap items-center gap-3 mt-2">
                      ...
                      {isOverselling && (
                        <span className="text-[8px] font-black text-red-600 bg-red-600/10 px-3 py-1 rounded-full uppercase animate-pulse flex items-center gap-1 border border-red-500/20">
                          <AlertTriangle size={12} /> Descuadre de Stock
                        </span>
                      )}
                    </div>
                  </div>
                  <button 
                    ...
                    className="p-3 text-slate-300 hover:text-red-500 transition-all hover:bg-red-500/5 rounded-2xl"
                  >
                    <Trash2 size={20}/>
                  </button>
                </div>
                ...
                <div className="flex justify-between items-center bg-slate-50 dark:bg-slate-950 rounded-[1.2rem] p-3 border border-slate-100 dark:border-slate-800">
                  <div className="flex items-center gap-4">
                    <button 
                      ...
                      className="p-2.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-brand-muted hover:text-brand-emerald transition-all shadow-sm active:scale-90"
                    >
                      <Minus size={16}/>
                    </button>
          ...
          {cart.length === 0 && (
            <div className="h-full flex flex-col items-center justify-center text-center py-20 opacity-20">
               ...
               <p className="text-[10px] font-black uppercase tracking-[0.5em]">Carrito Vacío</p>
            </div>
          )}
        </div>

        {/* FOOTER DEL CARRITO: TOTALIZADOR */}
        <div className="p-8 border-t border-brand-border bg-slate-50 dark:bg-slate-950/80 backdrop-blur-2xl">
          <div className="flex justify-between items-end mb-8">
            ...
            <div className="text-right">
              <p className="text-6xl font-black text-brand-emerald tracking-tighter transition-all">
          ...
          <Button 
            ...
            className="py-6 text-sm tracking-[0.4em] shadow-2xl shadow-emerald-500/30 rounded-3xl"
          >
            CONFIRMAR VENTA
          </Button>
        </div>
      ...
      <Modal isOpen={isCheckoutOpen} onClose={() => setIsCheckoutOpen(false)} title="FINALIZAR OPERACIÓN">
        <div className="space-y-8 p-2">
          <div className="text-center p-12 bg-emerald-500/5 dark:bg-slate-950 rounded-[3rem] border-2 border-brand-emerald/10 shadow-inner relative overflow-hidden">
             <div className="absolute top-0 right-0 w-32 h-32 bg-brand-emerald/5 blur-3xl rounded-full -mr-16 -mt-16 pointer-events-none"></div>
            <p className="text-[11px] font-black text-brand-muted uppercase tracking-[0.5em] mb-4">Monto a Liquidar</p>
            <p className="text-8xl font-black text-brand-emerald tracking-tighter animate-in zoom-in duration-500">
              ${total.toLocaleString()}
            </p>
          </div>
          ...
          <div className="space-y-3 px-4">
            ...
            <div className="relative group">
              <span className="absolute left-10 top-1/2 -translate-y-1/2 text-5xl font-black text-slate-200 dark:text-slate-800 transition-colors group-focus-within:text-brand-emerald">$</span>
              <input 
                type="number" 
                autoFocus 
                className="w-full pl-24 pr-10 py-10 text-7xl font-black bg-slate-50 dark:bg-slate-950 border-4 border-slate-100 dark:border-slate-800 rounded-[3rem] outline-none focus:border-brand-emerald text-slate-900 dark:text-white transition-all shadow-xl text-center"
          ...
          <div className={`mx-4 p-8 rounded-[2.5rem] border-2 flex justify-between items-center transition-all duration-500 ${change >= 0 ? 'bg-emerald-500/5 border-emerald-500/20 text-brand-emerald' : 'bg-red-500/5 border-red-500/20 text-red-500'}`}>
            <div className="flex items-center gap-3">
               {change >= 0 ? <CheckCircle2 size={24} /> : <AlertTriangle size={24} />}
               <span className="text-xs font-black uppercase tracking-[0.3em]">{change >= 0 ? 'Cambio al Cliente' : 'Faltante de Pago'}</span>
            </div>
            <span className="text-5xl font-black tracking-tighter">${Math.abs(change).toLocaleString()}</span>
          </div>

          <div className="flex gap-6 pt-4 px-4 pb-4">
            <Button 
              variant="secondary" 
              className="flex-1 py-6 rounded-3xl text-xs" 
