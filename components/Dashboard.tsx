...
const Dashboard: React.FC<DashboardProps> = ({ storeId }) => {
  ...
  const kpis = [
    { label: 'Venta Total en Caja', val: `$${ventaTotal.toLocaleString()}`, icon: ShoppingBag, color: 'text-brand-emerald' },
    { label: 'Costo Operativo Diario', val: `$${costoOperativo.toLocaleString()}`, icon: Target, color: 'text-orange-500' },
    { label: 'Utilidad Bruta', val: `$${utilidadBruta.toLocaleString()}`, icon: TrendingUp, color: 'text-brand-purple' },
    { label: 'Cobertura de Gastos', val: `${porcentajeEquilibrio.toFixed(0)}%`, icon: DollarSign, color: isCoveringCosts ? 'text-brand-emerald' : 'text-red-500' },
  ];
  ...
  return (
    <div className="space-y-8">
      ...
      {/* TARJETA PRINCIPAL - UTILIDAD DEL DÍA */}
      <div className="card-premium p-6 md:p-8 space-y-6 relative overflow-hidden bg-white dark:bg-slate-800 shadow-sm border border-slate-200 dark:border-slate-700 border-t-4 border-t-brand-purple">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 relative z-10">
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              <h4 className="text-[10px] font-black text-brand-muted uppercase tracking-[0.2em]">Utilidad del Día</h4>
              {!isOnline && <span className="text-[10px] text-orange-500 font-bold">(Offline)</span>}
            </div>

            <div className="flex items-baseline gap-2">
              <span className={`text-4xl md:text-6xl font-black tracking-tighter ${isCoveringCosts ? 'text-brand-emerald' : 'text-red-500'
                }`}>
          ...
          <div className="flex flex-col items-end gap-1">
            <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest flex items-center gap-2 ${isCoveringCosts ? 'bg-brand-emerald/10 text-brand-emerald' : 'bg-red-500/10 text-red-500'
              }`}>
              {isCoveringCosts ? <CheckCircle2 size={14} /> : <AlertTriangle size={14} />}
              {isCoveringCosts ? 'Cubriendo Gastos' : 'Bajo Punto de Equilibrio'}
            </span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 relative z-10">
          <div className="p-4 bg-slate-50 dark:bg-slate-900/50 rounded-2xl border border-slate-100 dark:border-slate-800">
            ...
            <p className="text-xl font-black text-slate-900 dark:text-white">${costoOperativo.toFixed(2)}</p>
          </div>
          <div className={`p-4 rounded-2xl border ${isProfitable
              ? 'bg-brand-emerald/5 dark:bg-brand-emerald/10 border-brand-emerald/20'
              : 'bg-red-500/5 dark:bg-red-500/10 border-red-500/20'
            }`}>
            <p className={`text-[10px] font-black uppercase tracking-widest mb-1 ${isProfitable ? 'text-brand-emerald' : 'text-red-500'
              }`}>Utilidad Neta</p>
            <p className={`text-xl font-black ${isProfitable ? 'text-brand-emerald' : 'text-red-500'
              }`}>${utilidadNeta.toLocaleString()}</p>
          </div>
        </div>
      </div>
      ...
      <div className="card-premium p-6 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-sm">
        <div className="flex justify-between items-center mb-4">
          <div>
            <h4 className="text-xs font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest">Cobertura de Gastos</h4>
            <div className="flex items-baseline gap-2 mt-1">
              <span className="text-2xl font-black text-slate-900 dark:text-white">${utilidadBruta.toLocaleString('es-MX', { minimumFractionDigits: 2 })}</span>
              <span className="text-xs font-bold text-slate-400">Ganancia / ${costoOperativo.toLocaleString('es-MX', { minimumFractionDigits: 2 })} Gastos</span>
            </div>
          </div>
          <div className="text-right">
            <span className={`text-xl font-black ${isCoveringCosts ? 'text-brand-emerald' : 'text-red-500'}`}>
        ...
        <div className="w-full bg-slate-100 dark:bg-slate-900 rounded-full h-3 overflow-hidden">
          <div
            className={`h-full rounded-full transition-all duration-1000 ease-out ${isCoveringCosts ? 'bg-brand-emerald' : 'bg-red-500'}`}
